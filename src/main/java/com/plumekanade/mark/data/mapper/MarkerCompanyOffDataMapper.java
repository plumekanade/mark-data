...
import com.plumekanade.mark.data.entity.MarkerCompanyOffData;
 ...
 */
@Mapper
public interface MarkerCompanyOffDataMapper extends BaseMapper<MarkerCompanyOffData> {

    @Insert("insert into marker_company_off_data values (null, #{companyName}, #{engName}, #{manageState}, #{legalName}, #{registerCapital}, #{realCapital}, #{establishDate}, #{verifyDate}, #{businessTerm}, #{province}, #{city}, #{district}, #{registrar}, #{creditCode}, #{taxpayerCode}, #{businessRegisterCode}, #{organizationCode}, #{taxpayerCredential}, #{insureNum}, #{companyType}, #{previousName}, #{registerAddress}, #{url}, #{contactPhone}, #{email}, #{businessScope}, #{companySize}, #{source}, #{industry}, #{firstClassify}, #{secondClassify}, #{thirdClassify}, #{lon}, #{lat})")
