...
public interface MarkerCompanyDataMapper extends BaseMapper<MarkerCompanyData> {
    ...
    @Select("select * from marker_company_data where id > #{maxId} order by id asc limit 0, #{size}")
    List<MarkerCompanyData> limitList(@Param("maxId") String maxId, @Param("size") int size);
}
